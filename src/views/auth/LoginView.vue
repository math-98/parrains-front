      ...
      <form method="post" action="#" @submit.prevent="submitForm">
        ...
        <div class="row mt-3 mb-2">
          ...
          <div class="col-9 d-grid">
            <input type="submit" class="btn btn-primary" value="Connexion" v-if="mode == 'login'" />
            <input type="submit" class="btn btn-primary" value="Envoyer" v-if="mode == 'reset'" />
          </div>
        </div>
      </form>
      <div class="text-center">
        <button class="btn btn-link small" v-if="mode == 'login'" @click="switchMode('reset')">
          Mot de passe oublié ?
        </button>
        <button class="btn btn-link small" v-if="mode == 'reset'" @click="switchMode('login')">
          Se connecter
        </button>
      </div>
    </div>
...
export default {
  mounted() {
    let toastElm = document.querySelector('#liveToast')
    if (!toastElm) {
      return
    }
    this.toast = new Toast(toastElm)
  },
  methods: {
    login() {
      useAuthStore()
